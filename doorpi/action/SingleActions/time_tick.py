...
# Deklariere die globale Variable korrekt
...
def time_tick(last_tick):
    global last_time_tick_second  # Stelle sicher, dass dies am Anfang der Funktion steht
    ...
    if datetime_now.hour != datetime_past.hour:
        ...
        for hour in HOUR_RANGE:
            if hour == datetime_now.hour:
                doorpi.DoorPi().event_handler(f'OnTimeHour{hour}', __name__)
    ...
    if datetime_now.minute != datetime_past.minute:
        ...
        for minute in MINUTE_RANGE:
            if minute == datetime_now.minute:
                doorpi.DoorPi().event_handler(f'OnTimeMinute{minute}', __name__)
    ...
    # Aktualisiere die globale Variable hier
...
def get(parameters):
    ...
    for minute in MINUTE_RANGE:
        doorpi.DoorPi().event_handler.register_event(f'OnTimeMinute{minute}', __name__)
    ...
    for hour in HOUR_RANGE:
        doorpi.DoorPi().event_handler.register_event(f'OnTimeHour{hour}', __name__)
