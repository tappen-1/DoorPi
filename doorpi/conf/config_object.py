...
class ConfigObject():
    ...
    def load_config(configfile, search_for_defaults=True):
        config = configparser.ConfigParser(allow_no_value=True)  # Updated for Python 3
    ...
    def set_value(self, section, key, value, log=True, password=False):
        ...
        password_friendly_value = "*******" if key == 'password' or password else value

        if key not in self.__sections[section]:
            if log:
    ...
    def get_string(self, section, key, default='', log=True, password=False, store_if_not_exists=True):
        ...
        if key.endswith('password') or password:
            if log:
                logger.debug("get_string for key %s in section %s (default: %s) returns %s", key, section, default, '*******')
        else:
            if log:
                logger.debug("get_string for key %s in section %s (default: %s) returns %s", key, section, default, value)
        return value
    ...
    def get_float(self, section, key, default=-1, log=True, store_if_not_exists=True):
        ...
        if log:
            logger.debug("get_float for key %s in section %s (default: %s) returns %s", key, section, default, value)
        return value
    ...
    def get_integer(self, section, key, default=-1, log=True, store_if_not_exists=True):
        ...
        if log:
            logger.debug("get_integer for key %s in section %s (default: %s) returns %s", key, section, default, value)
        return value
    ...
    def get_boolean(self, section, key, default=False, log=True, store_if_not_exists=True):
        ...
        if log:
            logger.debug("get_boolean for key %s in section %s (default: %s) returns %s", key, section, default, value)
        return value

    def get_list(self, section, key, default=None, separator=',', log=True, store_if_not_exists=True):
        if default is None:
            default = []
        value = self.get_string(section, key, str(default), log=False, store_if_not_exists=store_if_not_exists)
        value = value.split(separator) if value != '' else default
        if log:
            logger.debug("get_list for key %s in section %s (default: %s) returns %s", key, section, default, value)
        return value

    def get_sections(self, filter='', log=True):
        return_list = [section for section in self.__sections if filter in section]
        if log:
            logger.debug("get_sections returns %s", return_list)
        return return_list
    ...
    def get_keys(self, section, filter='', log=True):
        ...
        if section not in self.__sections:
            ...
            return_list = [key for key in self.__sections[section] if filter in key]
        if log:
            logger.debug("get_keys for section %s returns %s", section, return_list)
        return return_list

    def get_from_config(self, config, log=True):
        if log:
            logger.debug("get_from_config")
