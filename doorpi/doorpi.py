...
import html  # cgi.escape wurde in Python 3 durch html.escape ersetzt
import tempfile
import metadata
...
class DoorPi(metaclass=Singleton):  # Python 3 Syntax für Metaklassen
    __prepared = False
    ...
    def prepare(self, parsed_arguments):
        ...
        # register event-based actions from configfile
        ...
        # register actions for input pins
        ...
        if is_alive_led:
            self.event_handler.register_action('OnTimeSecondEvenNumber', f'out:{is_alive_led},HIGH,False')
            self.event_handler.register_action('OnTimeSecondUnevenNumber', f'out:{is_alive_led},LOW,False')
    ...
    def modules_destroyed(self):
        return len(self.event_handler.sources) <= 1 and self.event_handler.idle
    ...
    def parse_string(self, input_string):
        ...
        for key, value in mapping_table.items():
            parsed_string = parsed_string.replace(f"!{key}!", value)

        for key, value in self.additional_informations.items():
            parsed_string = parsed_string.replace(f"!{key}!", str(value))
